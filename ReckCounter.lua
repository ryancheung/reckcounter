...
function player_is_paladin()
    local localizedClass, englishClass = UnitClass("player");
    return englishClass == 'PALADIN'
end

function player_has_reckoning_talent()
    if not player_is_paladin() then
        return false
    end

    local numTalents = GetNumTalents(2);
    for i=1, numTalents do
        local nameTalent, icon, tier, column, currRank, maxRank= GetTalentInfo(2,i);
        if icon == "Interface\\Icons\\Spell_Holy_BlessingOfStrength" and currRank > 0 then
            return true
        end
    end
...
function reckcounter_initialize()
    reckcounter_core:Hide();
    if player_is_paladin() then
        DEFAULT_CHAT_FRAME:AddMessage(RECKCOUNTER_HIDE , RECK_RED, RECK_GREEN, RECK_BLUE);
    end
    reckcounter_display:SetText(RECKCOUNTER_RECKONING);
...
function reckcounter_OnEvent()
	...
	if (event == "CHAT_MSG_COMBAT_CREATURE_VS_SELF_HITS" or "CHAT_MSG_COMBAT_HOSTILEPLAYER_HITS" or "CHAT_MSG_SPELL_HOSTILEPLAYER_DAMAGE") then
		if (arg1 and not auto_attack) then
			if (reckcount < 4 and string.find(arg1, RECKCOUNTER_CRIT_YOU)) then
    ...
    if ( event == "PLAYER_ENTER_COMBAT" ) then
        ...
        if player_has_reckoning_talent() then
             if not reckcounter_core:IsShown() then
                 DEFAULT_CHAT_FRAME:AddMessage(RECKCOUNTER_SHOW , RECK_RED, RECK_GREEN, RECK_BLUE);
                 reckcounter_core:Show();
             end
        end
